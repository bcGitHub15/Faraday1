...
class IScan():
    ...
    def sendPlotsTo(self, p: ThreePlotWidget):
...
#            self.livePlotter.addLegend() # Must come BEFORE you plot the lines
            self.line1 = self.p1.plot(x=self.times, y=self.data[0, :],
                                                name='V1', pen='b', 
                                                symbol='o', symbolPen='b', 
                                                symbolBrush='b')
            self.line2 = self.p2.plot(self.times, self.data[1, :], 
                                                name='V2', pen='g', 
                                                symbol='o', symbolPen='g',
                                                symbolBrush='g')
            self.line3 = self.p3.plot(self.times, self.data[2, :],
                                                name='Vin', pen='r', 
    ...
    def stepScan(self):
        ...
        if nread > 0:
            print('+')
            if self.livePlotter:
                y = 0.005 * np.random.normal(size=10000)
                print(y[:3])
#                self.line1.setData(x=self.times, self.data[0, :])
                self.line1.setData(x=self.times, y=y)
